...
# UseEMpT: Set to 1 if the jet map is defined for energy deposited in ECAL (pT_EM vs pT). For jet map only, not photon!
        ...
        self.photon_file = self.open_root(cmssw_base + "/src/PhysicsTools/NanoAODTools/python/postprocessing/data/prefiring/" + photonroot)
        self.photon_map = self.get_root_obj(self.photon_file, photonmapname)

        self.jet_file = self.open_root(cmssw_base + "/src/PhysicsTools/NanoAODTools/python/postprocessing/data/prefiring/" + jetroot)
...
prefireCorr2016 = lambda : prefireCorr('L1prefiring_jetpt_2016BtoH.root', 'L1prefiring_jetpt_2016BtoH', 'L1prefiring_photonpt_2016BtoH.root', 'L1prefiring_photonpt_2016BtoH',0)
prefireCorr2017 = lambda : prefireCorr('L1prefiring_jetpt_2017BtoF.root', 'L1prefiring_jetpt_2017BtoF', 'L1prefiring_photonpt_2017BtoF.root', 'L1prefiring_photonpt_2017BtoF',0)
